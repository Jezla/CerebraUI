...
uvicorn cerebraui.main:app --host "%HOST%" --port "%PORT%" --forwarded-allow-ips "*" --workers %UVICORN_WORKERS% --ws auto
:: For ssl user uvicorn cerebraui.main:app --host "%HOST%" --port "%PORT%" --forwarded-allow-ips '*' --ssl-keyfile "key.pem" --ssl-certfile "cert.pem" --ws auto
