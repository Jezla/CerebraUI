...
from cerebraui.internal.db import Base, get_db
from cerebraui.utils.redis import get_redis_connection
...
class AppConfig:
    ...
    def __setattr__(self, key, value):
        if isinstance(value, PersistentConfig):
            ...
            if self._redis:
                redis_key = f"cerebraui:config:{key}"
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (CerebraUI + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.cerebraui.svc.cluster.local:11434"
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "cerebraui_collections"
)
# Pgvector
