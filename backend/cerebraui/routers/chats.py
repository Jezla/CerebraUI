...
from cerebraui.socket.main import get_event_emitter
from cerebraui.models.chats import (
    ChatForm,
    ChatImportForm,
    ChatResponse,
    Chats,
...
from cerebraui.models.tags import TagModel, Tags
from cerebraui.models.folders import Folders
...
from cerebraui.utils.access_control import has_permission
...
def _chat_cache_key(user_id: str, chat_id: str) -> str:
    return f"cerebraui:chat:{user_id}:{chat_id}"
...
async def get_chat_tags_by_id(
    ...
    redis = getattr(request.app.state.config, "_redis", None)
    cache_key = f"cerebraui:chat:tags:{user.id}:{id}"
