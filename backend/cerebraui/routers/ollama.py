...
from cerebraui.utils.payload import (
...
async def send_post_request(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=detail if detail else "CerebraUI: Server Connection Error",
        )
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="CerebraUI: Server Connection Error"
            )
        except Exception as e:
...
async def get_ollama_versions(request: Request, url_idx: Optional[int] = None):
    if request.app.state.config.ENABLE_OLLAMA_API:
        ...
        cache_key = (
            f"cerebraui:ollama:version:{url_idx if url_idx is not None else 'all'}"
        )
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "CerebraUI: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
