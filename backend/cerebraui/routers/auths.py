...
from cerebraui.models.otp import Otp, ResetPasswordForm
from cerebraui.models.auths import (
    AddUserForm,
    ApiKey,
    Auths,
    EmailResponse,
...
from cerebraui.models.otp import verifyOtpForm, verifyTokenForm, ResetPasswordForm
from cerebraui.models.users import Users

from cerebraui.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
...
from cerebraui.config import OPENID_PROVIDER_URL, ENABLE_OAUTH_SIGNUP, ENABLE_LDAP
...
from cerebraui.utils.access_control import get_permissions
...
async def send_otp_email(data: EmailResponse, request: Request):
    types = ["signin", "signup", "reset"]
    # validate type format
    if data.type not in types:
        raise HTTPException(400, detail="Type error")
...
@router.post("/verify_cf_token")
async def verify_cf_token(req: Request, payload: dict):
    ...
    try:
        async with ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    raise HTTPException(503, detail="Turnstile service unavailable")
                ...
                if not data.get("success"):
                    ...
                    raise HTTPException(
                        400, detail=f"Turnstile verification failed: {error_codes}"
                    )
                return {"success": True}

    except asyncio.TimeoutError:
        raise HTTPException(504, detail="Turnstile verification timeout")
    except Exception as e:
        log.error(f"Turnstile verification error: {str(e)}")
        raise HTTPException(503, detail="Turnstile verification service error")
